...
export const awsConfirmSignUpEmail = async (
  email,
  code
) => {
  // used to confirm signup with users 6-digit confirmation number
...
export const awsResendConfirmationCodeEmail =
  async email => {
    // Used to resend 6-digit confirmation code
    // Takes email
    // If unsuccessful, returns error object
...
export const awsSignIn = async (phone, password) => {
  ...
  console.log(
    `Phone number is +${phone} and password is ${password}`
  );
...
export const awsForgotPasswordSubmit = (
  phone,
  code,
  new_password
) => {
...
export const awsForgotPasswordSubmitEmail = (
  email,
  code,
  new_password
) => {
