...
import { BACKEND_API_GUESTY_BASE } from '@env';
import { BackendApi } from './backendApi';

const api = new BackendApi(
  BACKEND_API_GUESTY_BASE
);
...
export const getGuestByPhoneGuesty = async number => {
  ...
  return axios
    ...
    .catch(error => {
      console.log(
        '### Error with Guest by Phone Search...',
        {
...
export const getReservationsByPhoneGuesty = number => {
  ...
  return axios
    .get(
      `${api.backendUrl}/reservations/phone/${number}`,
      api.config
    )
    .then(response => {
    ...
    .catch(error => {
      console.log(
        '### Error Reservations by Phone Search...',
        {
...
export const getReservationByIDGuesty = async id => {
  ...
  return axios
    .get(
      `${api.backendUrl}/reservations/id/${id}`,
      api.config
    )
    .then(response => {
      return response.data;
    ...
    .catch(error => {
      console.log(
        '### Error with Reservation by Id Search',
        {
  ...
  return axios
    ...
    .catch(error => {
      console.log('### Error with Post New Task', {
        endpoint: `${api.backendUrl}/tasks/${JSON.stringify(
          data
        )}`,
        error
      });
...
export const deleteTask = async id => {
  return axios
    .delete(
      `${api.backendUrl}/tasks/${id}`,
      api.getConfig('DELETE')
    )
    .then(response => {
      return response.data;
    })
